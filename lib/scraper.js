...
async function collectWoori(source, errors = []) {
  ...
  try {
    ...
    const detail =
      ...
      (fallbackDetailSource?.period && isPeriodActive(fallbackDetailSource.period)
        ? { label: fallbackDetailSource.label, url: fallbackDetailSource.url }
        : null);
    ...
    return buildPolicyFromPage(
      ...
      targetPage,
      {
        ...
        ? "공식 할부 종합 안내"
        : detail?.url && fallbackDetailSource?.url === detail.url
          ? fallbackDetailSource.label
          : searchSource.label,
        url: targetUrl,
        period: detail?.period || (detail?.url && fallbackDetailSource?.url === detail.url
...
async function collectBc(source, errors = []) {
  const listSource = source.urls.find((candidate) => candidate.priority === "secondary") || source.urls[0];
