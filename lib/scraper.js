  ...
  return [
    {
      ...
      notes: [
        hasPharmacyBenefit
          ? `우리카드 공식 할부 종합 안내에서 약국 업종의 허용개월수를 자동 확인했습니다. 원문: ${sourceUrl}`
          : `우리카드 공식 할부 종합 안내에서 약국 업종을 확인했지만 무이자/부분무이자 허용개월수는 검색결과 없음으로 확인했습니다. 원문: ${sourceUrl}`
      ]
    }
  ];
}
    ...
    return [
      {
        ...
        minimumAmount: "없음",
        notes: [
          `IBK기업은행 공식 이벤트 게시글에서 약국 관련 무이자할부 조건을 찾지 못했습니다. 원문: ${sourceUrl}`
        ]
      }
    ];
  }
  ...
  return [
    {
      ...
      notes: [
        hasPharmacyBenefit
          ? `IBK기업은행 공식 이벤트 게시글에서 약국 관련 무이자할부 조건을 자동 확인했습니다. 원문: ${sourceUrl}`
          : `IBK기업은행 공식 이벤트 게시글에서 약국 업종을 확인했지만 무이자/부분무이자 조건은 검색결과 없음으로 확인했습니다. 원문: ${sourceUrl}`
      ]
    }
  ];
}
  ...
  return [
    {
      ...
      minimumAmount: "없음",
      notes: [
        `현대카드 공식 가맹점 업종별 무이자/부분 무이자 할부 상세 페이지에서 약국 관련 조건을 찾지 못했습니다. 원문: ${sourceUrl}`
      ]
    }
  ];
}
  ...
  return [
    {
      ...
      notes: [
        hasPharmacyBenefit
          ? `하나카드 공식 월간 무이자할부 혜택 글에서 약국 업종 조건을 자동 확인했습니다. 원문: ${sourceUrl}`
          : `하나카드 공식 월간 무이자할부 혜택 글에서 약국 업종을 확인했지만 무이자/부분무이자 조건은 검색결과 없음으로 확인했습니다. 원문: ${sourceUrl}`
      ]
    }
  ];
}
...
async function collectHana(source) {
  const errors = [];
  ...
  try {
    if (detailSource) {
      ...
      const policy = buildPolicyFromText(
        {
          ...source,
          industryOverrides: extractHanaIndustryPolicies(detailPage.text, detailSource.url)
        },
        detailPage.text,
        detailPage.html
      );
      ...
      return {
        ...policy,
        sourceLabel: detailSource.label,
    ...
    if (detail) {
      ...
      const policy = buildPolicyFromText(
        {
          ...source,
          industryOverrides: extractHanaIndustryPolicies(detailPage.text, detail.url)
        },
        detailPage.text,
        detailPage.html
      );
...
async function collectWoori(source) {
  const errors = [];
  const searchSource =
  ...
  try {
    ...
    const overrides = extractWooriIndustryPolicies(searchPage.text, searchSource.url);
    const policy = buildPolicyFromText(
      {
        ...source,
    ...
    return {
      ...policy,
      sourceLabel: overrides.length
        ? "공식 할부 종합 안내"
        : searchSource.label,
      sourceUrl: searchSource.url
    };
  } catch (error) {
...
async function collectIbk(source) {
  const errors = [];
  ...
  try {
    ...
    return {
      ...policy,
      sourceLabel: hasKeyword ? targetLabel : "공식 진행중 이벤트 검색결과 없음",
      sourceUrl: targetUrl
    };
  } catch (error) {
    errors.push(`${listSource.label}: ${error.message}`);
  ...
  return {
    ...
    ...source.fallback,
    status: "unavailable",
    sourceLabel: listSource.label || "원문",
...
async function collectHyundai(source) {
  const errors = [];
  ...
  for (const candidate of source.urls) {
    try {
      ...
      const policy = buildPolicyFromText(
        {
          ...source,
          industryOverrides: extractHyundaiIndustryPolicies(page.text, candidate.url)
        },
        page.text,
        page.html
      );
...
async function collectOne(source) {
  ...
  if (source.id === "hyundai") {
    const hyundaiPolicy = await collectHyundai(source);
    if (hyundaiPolicy) return hyundaiPolicy;
  }

  if (source.id === "hana") {
    const hanaPolicy = await collectHana(source);
    if (hanaPolicy) return hanaPolicy;
  }

  if (source.id === "woori") {
    const wooriPolicy = await collectWoori(source);
    if (wooriPolicy) return wooriPolicy;
  }

  if (source.id === "ibk") {
    const ibkPolicy = await collectIbk(source);
    if (ibkPolicy) return ibkPolicy;
  }
